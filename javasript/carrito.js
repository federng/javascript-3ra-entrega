...
let respuesta = prompt("¿Desea agregar una cerveza? SI - NO").toUpperCase()
while (respuesta != "NO" && respuesta != "no" ){
    agregarCerveza()
    respuesta = prompt("¿Desea agregar una cerveza? SI - NO").toUpperCase()
}

function agregarCerveza(){
    let nombreCerveza = prompt("Nombre de la cerveza");
    let precioCerveza = prompt ("Precio de la cerveza");
    let alcoholCerveza = prompt ("Graduación de alcohol ");
    cervezas.push(new Cervezas (nombreCerveza, precioCerveza, alcoholCerveza));
}
 */
